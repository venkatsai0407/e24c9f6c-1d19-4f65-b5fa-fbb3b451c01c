/*
SafeSteps - Disaster Safety Pro JavaScript
Complete functionality for interactive disaster safety education
Teachers can easily add new disasters by following the data structure below
*/

// ===== DISASTER SAFETY DATA =====
// Easy to modify - teachers can add new disasters here following the same format
const disasterData = {
    // NATURAL DISASTERS
    natural: [
        {
...
const keepSafeTips = document.getElementById('keep-safe-tips');
const helpOthersTips = document.getElementById('help-others-tips');
const funFactText = document.getElementById('fun-fact-text');

// ===== INITIALIZATION =====
// Wait for DOM to load before initializing the application
document.addEventListener('DOMContentLoaded', function() {
    console.log('SafeSteps - Disaster Safety Pro loaded successfully!');
    
    // Create disaster cards for each category
    createDisasterCards('natural', disasterData.natural);
    createDisasterCards('manmade', disasterData.manmade);
    createDisasterCards('personal', disasterData.personal);
    ...
    console.log('Application initialized with', 
        Object.values(disasterData).reduce((total, category) => total + category.length, 0), 
        'disaster guides loaded.');
});

// ===== DISASTER CARD CREATION =====
/**
 * Creates and displays disaster cards for a specific category
 * Teachers can modify this function to change how cards are displayed
 */
function createDisasterCards(category, disasters) {
    const container = document.getElementById(`${category}-disasters`);
    if (!container) {
        console.error(`Container for ${category} disasters not found`);
        return;
    }
    ...
    // Create card for each disaster
    disasters.forEach((disaster, index) => {
        const card = document.createElement('div');
        card.className = `disaster-card ${category}`;
        card.setAttribute('tabindex', '0'); // Make keyboard accessible
        card.setAttribute('role', 'button');
        card.setAttribute('aria-label', `Learn about ${disaster.title} safety`);
        ...
        card.innerHTML = `
            <div class="disaster-card-content">
                <div class="disaster-icon">${disaster.icon}</div>
                <h3>${disaster.title}</h3>
                <p>Click to learn safety tips</p>
            </div>
            <div class="hover-indicator"></div>
        `;

        // Add click and keyboard event listeners
        card.addEventListener('click', () => openModal(disaster, category));
        card.addEventListener('keypress', (e) => {
            if (e.key === 'Enter' || e.key === ' ') {
                e.preventDefault();
                openModal(disaster, category);
            }
        });
...
// ===== MODAL FUNCTIONALITY =====
/**
 * Opens the safety information modal for a specific disaster
 * Teachers can modify this to change how information is displayed
 */
function openModal(disaster, category) {
    if (isModalOpen) return; // Prevent multiple modals
    
    currentModal = disaster;
    isModalOpen = true;

    // Update modal header with category-specific styling
    const modalHeader = modal.querySelector('.modal-header');
    modalHeader.className = `modal-header ${category}-category`;

    // Update modal content
    modalIcon.textContent = disaster.icon;
    modalTitle.textContent = disaster.title;

    // Create safety tips
    createTipsList(keepSafeTips, disaster.keepSafe, 'primary');
    createTipsList(helpOthersTips, disaster.helpOthers, 'secondary');

    // Update fun fact
    funFactText.textContent = disaster.funFact;
    ...
    console.log(`Opened safety guide for: ${disaster.title}`);
}
...
/**
 * Creates a formatted list of safety tips
 * Teachers can modify this to change tip formatting
 */
function createTipsList(container, tips, colorClass) {
    container.innerHTML = '';
...
/**
 * Sets up modal event listeners for closing
 */
...
/**
 * Sets up keyboard navigation for better accessibility
 * Teachers can modify this to add more keyboard shortcuts
 */
function setupKeyboardNavigation() {
    document.addEventListener('keydown', (e) => {
        ...
        if (!isModalOpen) {
            ...
            if (focusedElement && focusedElement.classList.contains('disaster-card')) {
...
/**
 * Adds staggered animation delays to page elements
 * Teachers can modify timing here
 */
function addAnimationDelays() {
    // Add delays to feature cards
    const featureCards = document.querySelectorAll('.feature-card');
    featureCards.forEach((card, index) => {
        card.style.animationDelay = `${0.6 + (index * 0.2)}s`;
    });

    // Add delays to category headers
...
// ===== UTILITY FUNCTIONS =====
/**
 * Logs application statistics to console
 * Teachers can use this for debugging
 */
function logStatistics() {
    const totalDisasters = Object.values(disasterData)
        .reduce((total, category) => total + category.length, 0);
    
    console.log('=== SafeSteps Statistics ===');
    console.log(`Total Disasters: ${totalDisasters}`);
    console.log(`Natural Disasters: ${disasterData.natural.length}`);
    console.log(`Man-Made Disasters: ${disasterData.manmade.length}`);
    console.log(`Personal Emergencies: ${disasterData.personal.length}`);
    console.log('===========================');
}

/**
 * Validates disaster data structure
 * Teachers can use this to check if new disasters are formatted correctly
 */
function validateDisasterData() {
    const requiredFields = ['id', 'title', 'icon', 'keepSafe', 'helpOthers', 'funFact'];
    let isValid = true;

    Object.entries(disasterData).forEach(([category, disasters]) => {
        disasters.forEach((disaster, index) => {
            requiredFields.forEach(field => {
                if (!disaster[field]) {
                    console.error(`Missing ${field} in ${category}[${index}]: ${disaster.title || 'Unknown'}`);
                    isValid = false;
                }
            });

            // Validate arrays have correct length
            if (disaster.keepSafe && disaster.keepSafe.length !== 10) {
                console.warn(`${disaster.title} has ${disaster.keepSafe.length} 'keepSafe' tips (expected 10)`);
            }
            if (disaster.helpOthers && disaster.helpOthers.length !== 10) {
                console.warn(`${disaster.title} has ${disaster.helpOthers.length} 'helpOthers' tips (expected 10)`);
            }
        });
    });

    if (isValid) {
        console.log('✅ All disaster data is valid!');
    }
...
// Teachers can uncomment these lines if they want to test functions in browser console
// window.SafeSteps = {
//     disasterData,
//     openModal,
//     closeModal,
...
// Log statistics on load (for development)
setTimeout(() => {
    logStatistics();
    validateDisasterData();
}, 1000);
...
/*
=== CUSTOMIZATION GUIDE FOR TEACHERS ===

1. ADDING NEW DISASTERS:
   - Add new disaster objects to the appropriate category in disasterData
   - Follow the exact structure: id, title, icon, keepSafe (10 items), helpOthers (10 items), funFact
   - Use student-friendly language and short sentences

2. MODIFYING EXISTING DISASTERS:
   - Find the disaster in disasterData object
   - Update any field while maintaining the structure
   - keepSafe and helpOthers should always have exactly 10 tips each

3. CHANGING COLORS:
   - Modify the CSS custom properties in disaster_safety_pro.css
   - Update gradient variables for different category colors

4. ADDING ANIMATIONS:
   - Modify animation classes in CSS
   - Adjust animation-delay values in addAnimationDelays() function

5. KEYBOARD SHORTCUTS:
   - Modify setupKeyboardNavigation() to add new shortcuts
   - Current shortcuts: Arrow keys (navigation), Enter/Space (select), Escape (close)

6. DEBUGGING:
   - Open browser console (F12) to see logs and statistics
   - Use validateDisasterData() to check data structure
   - Use logStatistics() to see app statistics

7. ACCESSIBILITY:
   - All elements have proper ARIA labels and keyboard navigation
   - Screen reader friendly with semantic HTML
   - High contrast colors for better visibility

EXAMPLE: Adding a new disaster
disasterData.natural.push({
    id: 'hurricane',
    title: 'Hurricane', 
    icon: '🌀',
    keepSafe: [
        'Tip 1 for staying safe...',
        // ... 8 more tips ...
        'Tip 10 for staying safe...'
    ],
    helpOthers: [
        'Tip 1 for helping others...',
        // ... 8 more tips ...
        'Tip 10 for helping others...'
    ],
    funFact: 'Interesting fact about hurricanes!'
});
*/